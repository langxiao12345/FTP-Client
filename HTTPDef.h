...
typedef struct tagClientInf			
{
	...
	char			pszIP[MAX_IP_LENGTH + 1]; // 客户端连接IP地址,本来可以有hSocket取得,但是需要用到的地方太多了,另外保存一份提高效率.
...
public:
	...
	virtual void OnNewConnection(const char *pszIP, unsigned int nPort, BOOL bRefused, BOOL bKicked) = 0;

	// 连接端口时调用
	virtual void OnConnectionClosed(const char *pszIP, unsigned int nPort, REMOVE_REASON rr, __int64 nByteSent, unsigned int nTimeUsed) = 0;

	// 数据发送完成时调用
	virtual void OnDataSent(const char *pszIP, unsigned int nPort, unsigned int nBytesSent) = 0;

	// 接收到数据时调用
	virtual void OnDataReceived(const char *pszIP, unsigned int nPort, unsigned int nBytesReceived) = 0;

	// 处理完客户端请求,在发送回应到客户端前调用.
	virtual void OnRequested(const char *pszIP, unsigned int nPort, const char *pszUrl, HTTP_METHOD hm, SERVER_CODE sc) = 0;
};
