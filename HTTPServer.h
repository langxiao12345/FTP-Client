...
protected:
	...
	PCLIENTINF newClientDesc(SOCKET hSocket, const std::string &strIP, unsigned int nPort); // 初始化一个客户描述符.
	...
	BOOL mapServerFile(const std::string &strUrl, std::string &strFilePath); //把URL映射为服务器上的完整文件名.
	void lock();
	void unlock();
