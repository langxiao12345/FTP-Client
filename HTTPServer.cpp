...
PCLIENTINF CHTTPServer::newClientDesc(SOCKET hSocket, const std::string &strIP, unsigned int nPort)
{
	...
	_tcscpy(pClientInf->pszIP, strIP.c_str()); // 记录远程IP和端口.
...
HTTP_SERVER_ERROR_TYPE CHTTPServer::Run(PHTTPSTARTDESC pStartDesc)
{
	...
	// 创建默认文件名列表
	std::string strDftFileNames(pStartDesc->szDefaultFileName);
	...
	while( (stNext = strDftFileNames.find(L',', st)) != std::wstring::npos )
	{
		if(stNext > st)
		{
			std::string strDftFileName =  strDftFileNames.substr(st, stNext - st);
...
void CHTTPServer::doStop()
{
	...
	if(m_pServiceInf)
	{
		...
		for( int ii = 0; ii < nThreadCount; ++ii)
		{
			...
			PostQueuedCompletionStatus(m_pServiceInf->hCompletionPort, 0, 0, NULL);		// 3个参数均为0时,表示退出. 每个线程处理一个包
		}
...
BOOL CHTTPServer::OnAcceptEx(BOOL bSkeep)
{
	...
	// 获得远程IP和端口
	std::string strClientIP = inet_ntoa(clientAddr.sin_addr);
	unsigned int nClientPort = ntohs(clientAddr.sin_port);
...
int CHTTPServer::OnClose(PCLIENTINF pClientInf, REMOVE_REASON reason)
{
	...
	std::string strClientIP(pClientInf->pszIP);
	...
	while(true)
	{
		...
		else
		{
			...
			if(hSocket == ServiceInf.pHttpserver->m_sockListen)
			{
				// 侦听套接字可读
				CString str ;
				str.Format(_T("线程 %d The Web Server Accept a Client Connected!\r\n"),ServiceInf.nThreadIndex);
			...
			else
			{
				...
				else
				{
					...
					if(pSockInf->nOprationType == opp_recv)
					{
						// 接收操作.
						CString str ;
						str.Format(_T("线程 %d The Web Server Recv some data !\r\n"),ServiceInf.nThreadIndex);
						OutputDebugString(str);
					...
					else if(pSockInf->nOprationType == opp_send)
					{
						// 发送操作完成
						CString str ;
						str.Format(_T("线程 %d The Web Server Send the data !\r\n"),ServiceInf.nThreadIndex);
						OutputDebugString(str);
...
int CHTTPServer::OnRecv(PCLIENTINF pClientInf, DWORD dwBytesTransferred)
{
	...
	if(dwBytesTransferred != pClientInf->pRequest->PushData(pClientInf->WSABuf.buf, dwBytesTransferred) || pClientInf->pRequest->IsEnd())
	{
		...
		int nPopSize = pClientInf->pResponse->PopData(pClientInf->WSABuf.buf, MAX_SOCKBUFF); 
		ASSERT(nPopSize > 0);
...
void CHTTPServer::OnRequest(PCLIENTINF pSockInf)
{
	...
	std::string strUrlObject("");
	std::string strServerFilePath("");
	...
	if(strUrlObject.back() == L'/')
	{
		...
		else
		{
			...
			{
				std::string strDftFilePath(strServerFilePath);
				strDftFilePath += *iter;
...
BOOL CHTTPServer::mapServerFile(const std::string &strUrl, std::string &strFilePath)
{
	// 获得根目录
	...
	// 获得完整路径
	strFilePath += strUrl;
	for(std::string::iterator iter = strFilePath.begin(); iter != strFilePath.end(); ++iter)
	{
