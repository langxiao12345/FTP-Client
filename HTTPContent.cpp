...
BOOL CHTTPContent::OpenFile(const char* pszFileName)
{
	__int64 lFrom = 0;
	__int64 lTo = -1;
...
BOOL CHTTPContent::OpenFile(const char* pszFileName, __int64 &lFrom, __int64 &lTo)
{
...
BOOL CHTTPContent::OpenDir(const std::string &strUrl, const std::string &strFilePath)
{
	if(OPEN_NONE != m_nOpenType) return FALSE;
	ASSERT(m_pData == NULL);
	...
	intptr_t findRet = 1/*_wfindfirst64(strFullName.c_str(), &fileinfo)*/;
	if( -1 != findRet )
	{
		do 
		{
