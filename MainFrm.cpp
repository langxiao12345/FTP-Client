...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	_tcscpy(m_IconData.szTip, _T(" HTTP Server - by  C++ Studio"));
	...
	{
		LOGGER_CINFO(theLogger, _T(" HTTP Server 启动中...\r\n"));
		OnStart();
	}
	else
	{
		LOGGER_CINFO(theLogger, _T(" HTTP Server 准备就绪.\r\n"));
	}
...
std::string CMainFrame::getLogFileName()
{
...
void CMainFrame::OnStart()
{
	...
	if(Dlg.DoModal() == IDOK)//是否打开成功
	{
	    ...
	    FileTitle=FilePathName.Right(FilePathName.GetLength()-1-pos);
	}
	else//打开失败处理
	{
		//打开失败处理
	...
	urlObject= FileTitle;
	memcpy(_myURL,g_HTTP_Head,17);
	memcpy(_myURL+17,urlObject.c_str(),urlObject.size());
	LOGGER_CINFO(theLogger, _T(" HTTP Server URL is %s \r\n"),_myURL);
	...
	if( SE_SUCCESS == m_Server.Run(&startDesc))
	{
		...
		LOGGER_CINFO(theLogger, _T(" HTTP Server 启动成功,根目录[%s],端口[%d],最大连接数[%d].\r\n"), strRoot, startDesc.nPort, startDesc.nMaxConnection);
	}
	else
	{
		LOGGER_CINFO(theLogger, _T(" HTTP Server 启动失败,已经在运行或者端口[%d]被占用..\r\n"), startDesc.nPort);
	}
}
...
void CMainFrame::OnStop()
{
	...
	{
		ResetStatus();
		LOGGER_CINFO(theLogger, _T(" HTTP Server 停止服务.\r\n"));
	}
	else
	{
...
LRESULT CMainFrame::OnConnectionNumber(WPARAM w, LPARAM l)
{
	...
	char szConnectionsText[200] = {L'\0'};
	sprintf(szConnectionsText, _T("目前有: %d个连接"), nConnectionNumber);
	m_wndStatusBar.SetPaneText(1, szConnectionsText, TRUE);
	return 0;
}
...
void CMainFrame::OnConnectionClosed(const char *pszIP, unsigned int nPort, REMOVE_REASON rr, __int64 nByteSent, unsigned int nTimeUsed)
{
	// 需要同步
	lock();
	SetConnectionsNumber(m_nTotalConnections - 1);
	...
	switch(rr)
	{
	case RR_CLIENTCLOSED: { strReason = ("客户端关闭了连接"); break; }
	case RR_SENDCOMPLETE: { strReason  = ("数据发送完毕"); break; }
	case RR_RECV_FAILED: { strReason = ("接收失败"); break; }
	case  RR_SEND_FAILED: { strReason = ("发送失败"); break; }
	case RR_DEAD: { strReason = ("不活跃连接"); break; }
	case RR_SESSION_TIMEOUT: { strReason= ("会话超时"); break; }
	default: { strReason = ("未知"); break; }
	}
...
void CMainFrame::OnRequested(const char *pszIP, unsigned int nPort, const char *pszUrl, HTTP_METHOD hm, SERVER_CODE sc)
{
	// 只写一下日志就可以了.
